...
export const TokenApiAxiosParamCreator = function (configuration?: Configuration) {
    return {
        ...
        revokeToken: async (options: RawAxiosRequestConfig = {}): Promise<RequestArgs> => {
            const localVarPath = `/api/v1/users/token/revoke`;
            // use dummy base URL string because the URL constructor only accepts absolute URLs.
            const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
            let baseOptions;
            if (configuration) {
...
export const TokenApiFp = function(configuration?: Configuration) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        async revokeToken(options?: RawAxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<void>> {
            const localVarAxiosArgs = await localVarAxiosParamCreator.revokeToken(options);
            const index = configuration?.serverIndex ?? 0;
            const operationBasePath = operationServerMap['TokenApi.revokeToken']?.[index]?.url;
            return (axios, basePath) => createRequestFunction(localVarAxiosArgs, globalAxios, BASE_PATH, configuration)(axios, operationBasePath || basePath);
        },
    }
};
...
export class TokenApi extends BaseAPI {
    ...
    public revokeToken(options?: RawAxiosRequestConfig) {
        return TokenApiFp(this.configuration).revokeToken(options).then((request) => request(this.axios, this.basePath));
    }
}
